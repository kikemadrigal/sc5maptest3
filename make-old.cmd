...
:preparar_archivos_fuente
    ...
    for /R objects  %%a in (*.bas) do (
        type "%%a" >> objects/gametemp.asc & del /F /Q "%%a")  

    rem con msxbadignifier le quitamos las líneas,  creamos etiquetas para no recordar líneas,
    rem creamos variables largas
    start /wait tools\msxbadig\msxbadig.py gametemp.asc -tt o
    rem También le podemos quitar solo los comentarios con deletecommets
    rem start /wait java -jar tools/deletecomments/deletecomments.jar objects/gametemp.asc
    rem También l epodemos quita los comentarios con type, escribe type /? y find /? paa más ayuda
    rem type objects\gametemp.asc | find /V  "1 '"  > objects\game.asc
    del /F /Q objects\gametemp.asc
    echo .
    echo Comentarios eliminados y creado game.asc

    rem convrtirmos el texto ASCII a basic tokenized (que elimina los espacios en blanco y lo mejora)
    rem necesitas pyton 3.8 instalado en tu sistema 
    start /wait tools\msxbatoken\msxbatoken.py game.asc
